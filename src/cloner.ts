...
import {
  Guild as SelfGuild,
  GuildTextBasedChannel as SelfGuildTextBasedChannel,
  Message as SelfMessage,
  TextChannel
...
export class Cloner {
  ...
  async clone(id: string): Promise<GuildBasedChannel> {
    if (storage.channels.has(id)) {
      const mapped = storage.channels.get(id);
    ...
    if (!perms?.has("VIEW_CHANNEL")) {
      storage.channels.delete(channel.id);
      saveStorage();
      return null;
    }
    ...
    if (!FORCED_TYPES.includes(Constants.ChannelTypes[channel.type])) {
      console.log(
        `[CLONER] Skipping ${channel.type} channel ${channel.name} (${channel.id}) in ${this.source.name}`
      );
      storage.channels.delete(channel.id);
      saveStorage();
      return null;
    ...
    const result = await this.target.channels.create(channel.name, {
      ...
      topic: channel.type === "GUILD_TEXT" ? channel.topic : undefined,
      reason: "Cloned from " + channel.name,
      parent: (await this.getCloned(channel.parent.id)).id,
      position:
        channel.type === ("GUILD_TEXT" || "GUILD_NEWS")
          ? (channel as TextChannel).position
          : undefined,
    });

    storage.channels.set(channel.id, result.id);
    saveStorage();

    console.log(
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
      `[CLONER] Clone of ${channel.type} channel ${channel.name} (${channel.id}) in ${this.source.name} to ${result.name} (${result.id}) in ${this.target.name} complete`
    );
    return result;
  }
  ...
  async reorder() {
    ...
    for (const channel of channels.values()) {
      if (!storage.channels.has(channel.id)) continue;
      const mappedId = storage.channels.get(channel.id);
  ...
  async getCloned(id: string): Promise<GuildBasedChannel | null> {
    if (!storage.channels.has(id)) {
      return await this.clone(id);
    }

    return await this.target.channels.fetch(storage.channels.get(id));
  }
  ...
  async purgeLegacy() {
    ...
    for (const id of channels) {
      ...
      console.log(
        `[CLONER] Purging legacy channel ${channel.name} (${channel.id}) in ${this.target.name}`
      );
  ...
  async sync() {
    ...
    this.webhookManager.webhooks.clear();
    ...
    for (const channel of this.source.channels.cache.values()) {
      ...
      if (!mapped?.name || !channel.name || mapped.name === channel.name)
        continue;
      ...
      try {
        ...
        console.log(
          `[CLONER] Failed to rename channel ${channel.name} (${channel.id}) to ${mapped.name} (${mapped.id}) in ${this.target.name}...`
        );
      }
    }
    ...
    if (this.source.channels.cache.size > 50) {
      console.log(
        `[SELF BOT] Skipping backlog for ${this.target.name} due to large channel count`
      );
      return;
    }
  ...
  async backlog(channel: SelfGuildTextBasedChannel) {
    ...
    try {
      ...
      const lastId = storage.backlog.get(channel.id);
      const lastMessage = await channel.messages.fetch(lastId);

      const messages = (await channel.messages.fetch({ limit: 50 })).sort(
        (a, b) => a.createdAt.getTime() - b.createdAt.getTime()
      );

      console.log(
        `[CLONER] Backlogging ${messages.size} for ${channel.name} (${channel.id}) in ${channel.guild.name}...`
      );
      ...
      console.log(
        `[CLONER] Backlogged ${messages.size} for ${channel.name} (${channel.id}) in ${channel.guild.name}...`
      );
    } catch (e) {
      console.error(
        `[CLONER] Failed to fetch backlog for ${channel.name} (${channel.id}) in ${channel.guild.name}...`,
        e
      );
    }
  }
  ...
  async cloneMessage(message: SelfMessage, mentions = true) {
    const channel: GuildTextBasedChannel = (await this.getCloned(
      message.channel.id
    )) as GuildTextBasedChannel;
    const webhook: Webhook = await this.webhookManager.getClient(channel);
    if (!channel) return;

    storage.backlog.set(channel.id, message.id);
    saveStorage();
    ...
    if (message.reference) {
      try {
        ...
        const reference = await (
          referencedChannel as SelfGuildTextBasedChannel
        ).messages.fetch(message.reference.messageId);
        if (reference) {
          content += `> ${reference.content
            .split("\n")
            .join("\n> ")}\n> Reply from ${reference.author.toString()}\n`;
        }
      } catch (e) {
        console.error(
          `[CLONER] Failed to fetch reference message for ${message.id} in ${message.guild.name}`,
          e
        );
      }
    }
    ...
    if (!content.length && !message.attachments.size && !message.embeds.length)
      return;

    await webhook.send({
      content:
        content.length > 2000
          ? content.slice(0, 1995) + "[...]"
          : content.length
          ? content
          : undefined,
